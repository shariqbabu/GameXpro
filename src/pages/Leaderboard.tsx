    ...
    <div className="p-4 lg:p-6 space-y-6">
      ...
      {!loading && sortedLeaders.length >= 3 && (
        ...
        >
          ...
          <div className="text-center">
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-gray-400 to-gray-500 flex items-center justify-center text-2xl font-bold mb-2 mx-auto ring-2 ring-gray-400/30">
              {sortedLeaders[1]?.photoURL ? <img src={sortedLeaders[1].photoURL} className="w-full h-full object-cover rounded-full" alt="" /> : sortedLeaders[1]?.displayName?.[0]}
            </div>
          ...
          {/* 1st Place */}
          <div className="text-center">
            <div className="w-20 h-20 rounded-full bg-gradient-to-br from-yellow-400 to-yellow-500 flex items-center justify-center text-3xl font-bold mb-2 mx-auto ring-4 ring-yellow-400/40 animate-pulse-glow">
              {sortedLeaders[0]?.photoURL ? <img src={sortedLeaders[0].photoURL} className="w-full h-full object-cover rounded-full" alt="" /> : sortedLeaders[0]?.displayName?.[0]}
            </div>
          ...
          {/* 3rd Place */}
          <div className="text-center">
            <div className="w-14 h-14 rounded-full bg-gradient-to-br from-orange-400 to-orange-500 flex items-center justify-center text-xl font-bold mb-2 mx-auto ring-2 ring-orange-400/30">
              {sortedLeaders[2]?.photoURL ? <img src={sortedLeaders[2].photoURL} className="w-full h-full object-cover rounded-full" alt="" /> : sortedLeaders[2]?.displayName?.[0]}
            </div>
      ...
      <div className="flex gap-1 bg-white/5 rounded-xl p-1">
        {[
          ...
          <button
            key={tab.value}
            onClick={() => setActiveTab(tab.value as 'wins' | 'earnings' | 'referrals')}
              ...
              return (
                ...
                >
                  ...
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center font-bold text-sm overflow-hidden flex-shrink-0">
                    {player.photoURL ? <img src={player.photoURL} className="w-full h-full object-cover" alt="" /> : player.displayName?.[0]}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <p className="text-white font-medium text-sm">{player.displayName}</p>
                      {isCurrentUser && <span className="text-xs bg-purple-500/20 text-purple-400 px-1.5 py-0.5 rounded">You</span>}
                    </div>
