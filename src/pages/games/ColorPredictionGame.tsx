...
export const ColorPredictionGame = () => {
  ...
  const [balance, setBalance] = useState(userProfile?.walletBalance || 1250);
  ...
  const handleRoundEnd = () => {
    ...
    if (currentBet) {
      const won = currentBet.color === res;
      const payout = won ? currentBet.amount * MULTIPLIERS[currentBet.color] : 0;
      if (won) {
        setBalance(prev => prev + payout - currentBet.amount);
        setShowWinAnim(true);
        toast.success(`🎉 You won ₹${payout.toFixed(0)}!`, { duration: 3000 });
        setTimeout(() => setShowWinAnim(false), 3000);
      } else {
        toast.error(`❌ You lost ₹${currentBet.amount}`);
      }
      setBetHistory(prev => [{
        round: roundNumber,
    ...
    setRoundHistory(prev => [{ id: `round${roundNumber}`, roundNumber, result: res, totalBets: Math.floor(Math.random() * 50) + 10 }, ...prev.slice(0, 19)]);
  ...
  const placeBet = () => {
    ...
    toast.success(`✅ Bet placed! ₹${betAmount} on ${colorConfig[selectedColor].label}`);
  };
          ...
          <AnimatePresence>
            {phase === 'result' && result && (
              <motion.div
                ...
                style={{ background: `rgba(var(--tw-${result === 'violet' ? 'violet' : result}-500-rgb), 0.15)` }}
              >
