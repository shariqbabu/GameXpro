...
const rankBadge = (rank: number) => {
  if (rank === 1) return { emoji: '🥇', bg: 'from-yellow-400 to-yellow-600', text: 'text-yellow-900' };
  if (rank === 2) return { emoji: '🥈', bg: 'from-gray-300 to-gray-500', text: 'text-gray-900' };
  if (rank === 3) return { emoji: '🥉', bg: 'from-orange-400 to-orange-600', text: 'text-orange-900' };
  return { emoji: `#${rank}`, bg: 'from-white/10 to-white/5', text: 'text-white/60' };
};
    ...
    <div className="space-y-6 pb-4">
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold font-sora gradient-text-gold">Leaderboard</h1>
        </div>
        <p className="text-white/50 text-sm">Top players competing for glory and cash prizes</p>
      </motion.div>
      ...
      <div className="flex gap-1 p-1 rounded-2xl bg-white/5 border border-white/8">
        {tabs.map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all ${activeTab === tab ? 'bg-gradient-to-r from-yellow-500 to-orange-500 text-white' : 'text-white/50 hover:text-white'}`}>
            {tab}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 items-end">
        ...
        >
          <div className="mb-2">
            ...
            <div className="w-5 h-5 rounded-full bg-gray-400 flex items-center justify-center text-xs font-bold text-gray-900 mx-auto -mt-2">2</div>
          </div>
          <div className="bg-gradient-to-b from-gray-400/20 to-transparent rounded-2xl p-3 border border-gray-400/20">
            ...
            <p className="text-xs text-gray-400 font-semibold">₹{top3[1]?.totalEarnings.toLocaleString()}</p>
            <p className="text-[10px] text-white/30">{top3[1]?.totalWins} wins</p>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="mb-2">
            <div className="w-18 h-18 rounded-full bg-gradient-to-br from-yellow-400 to-yellow-600 flex items-center justify-center text-3xl mx-auto shadow-2xl shadow-yellow-500/30 w-16 h-16">
              {top3[0]?.username?.[0] || 'P'}
            </div>
            <div className="w-6 h-6 rounded-full bg-yellow-400 flex items-center justify-center text-xs font-bold text-yellow-900 mx-auto -mt-3">1</div>
          </div>
          <div className="bg-gradient-to-b from-yellow-400/20 to-transparent rounded-2xl p-3 border border-yellow-400/30">
            ...
            <p className="text-sm text-yellow-400 font-bold">₹{top3[0]?.totalEarnings.toLocaleString()}</p>
            <p className="text-xs text-white/40">{top3[0]?.totalWins} wins</p>
          </div>
        </motion.div>
        ...
        >
          <div className="mb-2">
            ...
            <div className="w-5 h-5 rounded-full bg-orange-500 flex items-center justify-center text-xs font-bold text-white mx-auto -mt-2">3</div>
          </div>
          <div className="bg-gradient-to-b from-orange-500/20 to-transparent rounded-2xl p-3 border border-orange-500/20">
            ...
            <p className="text-xs text-orange-400 font-semibold">₹{top3[2]?.totalEarnings.toLocaleString()}</p>
            <p className="text-[10px] text-white/30">{top3[2]?.totalWins} wins</p>
          </div>
        </motion.div>
      </div>
        ...
        <div className="divide-y divide-white/5">
          {rest.map((player, i) => {
            ...
            return (
              ...
              >
                <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${badge.bg} flex items-center justify-center text-xs font-bold ${badge.text}`}>
                  {player.rank}
                </div>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-3 text-xs text-white/40 mt-0.5">
                    <span className="flex items-center gap-1"><Trophy className="w-3 h-3" /> {player.totalWins} wins</span>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <div className="flex items-center justify-end gap-1 mt-0.5">
                    ...
                    <span className="text-xs text-yellow-400">{(player.totalWins * 50).toLocaleString()} pts</span>
                  </div>
                </div>
              </motion.div>
            );
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/40">Keep playing to climb ranks!</p>
            </div>
          </div>
          <div className="text-right">
